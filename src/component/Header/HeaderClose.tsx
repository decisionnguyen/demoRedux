...
const Icon = styled.Image<{ color?: string }>`
  width: 24px;
  height: 24px;
  tint-color: ${p => p.color || p.theme.grey1};
`;
...
export const HeaderClose = memo(function HomeHeader(props: Props) {
    const {title, right, center, rightContainerStyle, containerStyle, backColor, withDivider, titleStyle} = props;
    const navigation = useNavigation();
    return (
        <Container style={containerStyle} withDivider={withDivider}>
            <Left>
                {/*<Icon source={require('./assets/arrow-left.png')} color={backColor}/>*/}
            </Left>
            <Center>
                {
                    center
